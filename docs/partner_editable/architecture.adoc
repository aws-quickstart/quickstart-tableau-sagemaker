...
* Amazon API Gateway to maintain and secure a REST API with two endpoints (GET /info and POST /evaluate).
* Two AWS Lambda functions, one attached to each API Gateway endpoint. When Tableau sends a request (for example, to retrieve a SageMaker inference) API Gateway calls the associated AWS Lambda function.
