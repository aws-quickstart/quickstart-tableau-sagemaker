...
* Two AWS Lambda functions, one attached to each API Gateway endpoint. When Tableau sends a request to a SageMaker endpoint (for example, to retrieve a SageMaker inference) API Gateway calls the associated AWS Lambda function.
