...
In the event that your ML model requires additional transformations, that logic can be applied in the "create_sagemaker_body" function, or in an additional function for pre-processing that reads the csv output from "create_sagemaker_body". This allows for additional transformations and presents the opportunity to easily integrate customizations into the functionality of the {partner-product-name} solution. 
